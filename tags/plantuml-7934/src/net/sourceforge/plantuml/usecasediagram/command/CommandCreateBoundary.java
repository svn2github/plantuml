...
public class CommandCreateBoundary extends AbstractCommandCreate {

	public CommandCreateBoundary(UsecaseDiagram usecaseDiagram) {
		super(usecaseDiagram, getRegexConcat());
	}

	static RegexConcat getRegexConcat() {
		return new RegexConcat(new RegexLeaf("^"), //
				new RegexLeaf("TYPE", "(boundary|control|entity)\\s+"), //
	...
	protected CommandExecutionResult executeArg(Map<String, RegexPartialMatch> arg) {
		return executeArg(arg, getType(arg.get("TYPE").get(0)));
	}

	private LeafType getType(String type) {
		if ("control".equalsIgnoreCase(type)) {
			return LeafType.CONTROL;
		}
		if ("entity".equalsIgnoreCase(type)) {
			return LeafType.ENTITY_DOMAIN;
		}
		return LeafType.BOUNDARY;
	}
