...
public class CommandLinkComponent extends SingleLineCommand2<ComponentDiagram> {
	...
	private Direction getDirection(RegexResult arg) {
		if (arg.get("AR_TO_RIGHT", 2) != null) {
			return StringUtils.getQueueDirection(arg.get("AR_TO_RIGHT", 2));
		}
		if (arg.get("AR_TO_LEFT", 3) != null) {
			return StringUtils.getQueueDirection(arg.get("AR_TO_LEFT", 3)).getInv();
		}
		return null;
	}
	...
	private CommandExecutionResult executePackageLink(RegexResult arg) {
		...
		final Link link = new Link(cl1, cl2, linkType, arg.get("END", 0), queue.length());
		getSystem().addLink(link);
		return CommandExecutionResult.ok();
	}
